/**
 * This was taken from the HN site on 8/30/2023 as a precaution to protect
 * against potential breaking changes in the future. It is not currently in use
 * but could be used in the content script if HN makes changes to their JS that
 * do not play well with how things are built today.
 */
...
function byClass(el, cl) {
  return el ? el.getElementsByClassName(cl) : [];
}
function byTag(el, tg) {
  return el ? el.getElementsByTagName(tg) : [];
}
function allof(cl) {
  return byClass(document, cl);
}
function classes(el) {
  return (el && el.className && el.className.split(" ")) || [];
}
function hasClass(el, cl) {
  return afind(cl, classes(el));
}
...
function upclass(el, cl) {
  return uptil(el, function (x) {
    return hasClass(x, cl);
  });
}
function html(el) {
...
function apos(x, a) {
  return typeof x == "function"
    ? posf(x, a)
    : Array.prototype.indexOf.call(a, x);
}
function afind(x, a) {
  var i = apos(x, a);
  return i >= 0 ? a[i] : null;
...
function acut(a, m, n) {
  return Array.prototype.slice.call(a, m, n);
}
function aeach(fn, a) {
...
function vis(el, on) {
  if (el) {
    (on ? remClass : addClass)(el, "nosee");
  }
}
function setshow(el, on) {
  (on ? remClass : addClass)(el, "noshow");
}
function noshow(el) {
  setshow(el, false);
}
...
function onop() {
  return attr(byTag(document, "html")[0], "op");
}
...
function moreurl() {
  return allof("morelink")[0].href;
}
function morenext() {
  return tonum(moreurl().split("next=")[1]);
}
